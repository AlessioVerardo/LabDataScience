...
[![video presentation](./figs/title.png)](https://drive.google.com/file/d/1tnHstdgAqAuSV7zLT1WY1WFdTDCOWkzb/view?usp=sharing)  

<div>
    <div>
        <img src="./figs/input.png" alt="input form" width="500"/>
        <div><i>Input menu</i></div>
    </div>
    <div>
        <img src="./figs/conn.png" alt="connections menu" width="500"/>
        <div><i>Connections menu</i></div>
    </div>
    <div>
        <img src="./figs/map.png" alt="map" width="500"/>
        <div><i>Map</i></div>
    </div>
</div>


To use the route planner, run the `notebooks/demo.py` file. At the bottom of the notebook you will see the input form. Fill in the __From__ and __To__ stations, the desired arrival time and certainty. Once these parameters are set, click the __Find Routes__ button. In about one minute, the connections timetable and the map will appear on the screen. Each tab of the connections timetable contains the detailed route description. To see the intermediate stations between two stops, unwrap the accordion in the route's tab. **You might want to lower the certainty of your trips as our planner is still a bit pessimistic on the found paths.**

[Video presentation](https://drive.google.com/file/d/1tnHstdgAqAuSV7zLT1WY1WFdTDCOWkzb/view?usp=sharing)

[top](#Content)
----
## Work separation
- Basis infrastructure graph :
    - Extraction of nodes in the Zürich vicinity : Alessio + Cyrille
    - Extraction of edges (transport, walking, transfer) : Alessio + Cyrille
    - Processing of CFF data and delay modeling : Alessio + Cyrille
    - Linking the delays data with the basis infrastructure graph : Alessio + Cyrille
- Processing of a trip query :
    - Pruning of the basis graph based on required arrival time : Stephane
    - Pruning of the basis graph based on maximal trip duration : Stephane
    - Converting the pruned distributed graph into a local Network graph : Stephane + Cyrille + Emna
    - Finding shortest paths in the local graph : Cyrille
    - Prepare data for display: Alexandera + Stephane
- User Interface :
    - Form to enter the starting point and destination, desired arrival hour, required confidence : Alexandera
    - Tabular display of the results : Alexandera
    - Map to display the different trips : Alexandera

File description:
- `confidence.py`: Python file providing methods to compute probabilities and confidence along a given path.
- `delays.ipynb`: Notebook where we build and fit the models for the delays of the different mean of transport, stations and hour of the day. 
- `graph_processing.ipynb`: Notebook where we extract the nodes and the edges that we will use to build our transportation network graph. This will form the basis for the queries. Note that this notebook should only be run once to create the basic graphs and the subsequent queries don't need to run it again. 
- `graph_building.ipynb`: Merge of the pre-processed edges and nodes to build our full graph.
- `route_planner.py`: Notebook (Jupytext) where we process a route planning query, build the query-tailored networkx graph and return the shortest routes.
- `demo.py`: Notebook (Jupytext) containing the User Interface relying on the previous files to process a query. 

Please open the jupytext files as jupyter notebooks by right clicking in the file explorer and choosing **Open with Notebook**.

The `demo.py` is divided into 5 parts, namely:
- **Part I - Setup and Importing the graph data**: Here we load the preprocessed edges we have on HDFS.
- **Part II - Pruning the graph**: Define functions to prune the graph from edges and nodes that we know won't satisfy a given query, based on assumptions we made.
- **Part III - Constructing the graph**: We save the filtered graph data on HDFS, since they are still too big to store locally or by using Spark. So we define functions to retrieve the filtered graph from HDFS and build the Graph locally using `NetworkX`.
- **Part IV - Interpret the query function**: Here we setup the pipeline that takes into account all the previous manipulations, and defines a function to process a query and search the graph for the shortest routes. We also put an example at the end of the `search_graph` to see its output. Please uncomment it to see the results.
- **Part V - Graphical Interface**: Here you see the result of all this work in a nice user interface. We define functions to setup the graphical interface to input the query and see the output of the routes in a window with accordions for each suggested trip, as well as display the routes on the map interactively.
---- 

## Limitations
We limited ourselves to the following assumptions 
- Maximal acceptable waiting time: 15min
- Maximal trip length in Zurich area: 2h
- Radius of 15km around Zurich main station (with 2.5km margin)
----

## Disclaimer
- Running time of 1min per query.
- Time might be higher for the first query as it needs to start the Spark session.
- 15min waiting assumption might break some long range query (for trip outside of Zurich centrum).
- **You might want to lower the certainty of your trips as our planner is still a bit pessimistic on the found paths.**
